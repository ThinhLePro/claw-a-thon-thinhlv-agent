...
# Safety: restrict file operations to a workspace directory
WORKSPACE_DIR = os.environ.get("AGENT_WORKSPACE", "/tmp/agent-workspace")
os.makedirs(WORKSPACE_DIR, exist_ok=True)

MAX_FILE_SIZE = 10 * 1024 * 1024  # 10MB read limit
...
def read_file(file_path: str) -> str:
    ...
    Use this to read configuration files, log files, scripts, or any text file
    that has been placed in the agent workspace.

    Args:
        file_path: Path to the file (relative to workspace, or absolute if within workspace).
    """
...
def write_file(file_path: str, content: str) -> str:
    ...
    Use this to save generated configurations, scripts, reports, diagrams,
    runbooks, or any other output.

    Args:
        file_path: Path for the output file (relative to workspace, or absolute if within workspace).
...
def list_workspace_files(subdirectory: str = "") -> str:
    ...
    Args:
        subdirectory: Optional subdirectory within workspace to list.
    """
...
def http_request(url: str, method: str = "GET", headers: str = "", body: str = "") -> str:
    ...
    Use this to interact with monitoring systems (Prometheus, Grafana, ELK),
    REST APIs, webhooks, or any HTTP service.

    Args:
        url: The full URL to request.
        method: HTTP method (GET, POST, PUT, DELETE, PATCH).
        headers: Optional headers as key:value pairs, one per line.
    ...
    try:
        ...
        # Truncate response for LLM safety
