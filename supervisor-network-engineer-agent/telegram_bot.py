...
def start_telegram_bot(process_message_fn, bot_token: str):
    ...
    async def cmd_help(update: Update, context) -> None:
        ...
        help_text = (
            ...
            "/start — Bắt đầu\n"
            "/help — Hướng dẫn\n"
            "Bắt đầu bằng tin nhắn để tự động khởi tạo ticket chẩn đoán! 🚀"
        )
    ...
    application.add_handler(CommandHandler("new", handle_message))
    application.add_handler(CommandHandler("newchat", handle_message))
    application.add_handler(CommandHandler("reset", handle_message))
